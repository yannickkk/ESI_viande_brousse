...
ui <- navbarPage(windowTitle = "ESI Pointe Noire bushmeat survey", title=div(tags$a(img(src="esi.gif", style="margin-top: -15px",height ="50" ,width ="179" ), href="https://www.facebook.com/esicongo", target ="_blank")),
                 tabPanel("Data Viewer",
                          ...
                          sidebarLayout(
                            sidebarPanel(
                              ...
                              selectInput("taxa","Choice taxa (single or multiple)",paste("whole taxa",dat$species),multiple = TRUE,selected ="whole taxa"),
                              selectInput("var3",label= "Choice market (single or multiple)",choices = c("whole markets", levels(dat$LIEU)),selected = "whole markets",multiple=TRUE),
                            ...
                            mainPanel( 
                              ...
                              h6("missing data for occurences have been transformed in 1"),
                              width = 10)
                            ###################
                            ),
                 ...
                 tabPanel("Spatial Viewer",
                          sidebarLayout(
                            sidebarPanel(
                              ...
                              selectInput("taxa2","Choice taxa (single or multiple",choices = c("whole taxa",dat$species),multiple = TRUE,selected ="whole taxa"),
                              selectInput("market2",label= "Choice market (single or multiple)",choices = c("whole markets", levels(dat$LIEU)),selected = "whole markets",multiple=TRUE),
                 ...
                 tabPanel("Species Information",
                          sidebarLayout(
                            sidebarPanel(
                              selectInput("species",label = "Species",choices = c("",levels(dat$name)),selected = ""),
